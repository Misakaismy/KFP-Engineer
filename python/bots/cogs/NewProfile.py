...
class NewProfile(commands.Cog):
    ...
    async def profile_on_message(self, message:Message):
        ...
        if membeInDb.rank != rank:
            ...
            role_dict=load_json_file(str(message.guild.id)+'_rank_roles.json')
            if rank%20 == 0:
                if role_dict != False:
                    for i in rande(100, 19, -20):
                        if i == rank:
            ...
            if rank == 1:
                ...
                if rankup_channel_id != None:
                    await message.guild.get_channel(rankup_channel_id).send(embed= t_rmbed)
        self.db.increase_normal_coin(message.guild.id, message.author.id, increaseNumber)
